                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="row float-right">
                                <a href="{{ route('dashboard.developer.details.create', $developer->id) }}" class="btn btn-block btn-primary">
                        ...
                        <div class="card-body">
                            <table class="table table-hover text-nowrap table-striped datatable">
                                ...
                                <tbody>
                                    @foreach ($developer->details as $key => $value)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $value->name }}</td>
                                            <td>{{ $value->value }}</td>
                                            ...
                                            <td class="project-actions text-right">
                                                <form method="POST"
                                                    action="{{ route('dashboard.developer.details.destroy', [$developer->id,$value->id]) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="btn btn-info btn-sm"
                                                        href="{{ route('dashboard.developer.details.edit', [$developer->id,$value->id]) }}">
