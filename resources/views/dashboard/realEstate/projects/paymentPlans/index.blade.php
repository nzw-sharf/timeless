                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="row float-right">
                                <a href="{{ route('dashboard.project.paymentPlans.create', $project->id) }}" class="btn btn-block btn-primary">
                        ...
                        <div class="card-body">
                            <table class="table table-hover text-nowrap table-striped datatable">
                                ...
                                <tbody>
                                    @foreach ($project->paymentPlans as $key => $value)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $value->name }}</td>
                                            <td>{{ $value->value }}</td>
                                            ...
                                            <td class="project-actions text-right">
                                                <form method="POST"
                                                    action="{{ route('dashboard.project.paymentPlans.destroy', [$project->id,$value->id]) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="btn btn-info btn-sm"
                                                        href="{{ route('dashboard.project.paymentPlans.edit', [$project->id,$value->id]) }}">
